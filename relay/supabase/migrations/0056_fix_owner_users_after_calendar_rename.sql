...
begin
  ...
  select jsonb_build_object(
    'overview', jsonb_build_object(
      ...
      'calendar_connected', exists(select 1 from public.calendar_integrations ci where ci.user_id = p.id),
      'calendar_accounts', (select count(*) from public.calendar_integrations ci where ci.user_id = p.id),
