...
drop policy if exists "relay_calendar_events_select_own" on public.relay_calendar_events;
create policy "relay_calendar_events_select_own"
  on public.relay_calendar_events for select to authenticated
  using ((select auth.uid()) = user_id);
drop policy if exists "relay_calendar_events_insert_own" on public.relay_calendar_events;
create policy "relay_calendar_events_insert_own"
  on public.relay_calendar_events for insert to authenticated
  with check ((select auth.uid()) = user_id);
drop policy if exists "relay_calendar_events_update_own" on public.relay_calendar_events;
create policy "relay_calendar_events_update_own"
  on public.relay_calendar_events for update to authenticated
  using ((select auth.uid()) = user_id)
  with check ((select auth.uid()) = user_id);
drop policy if exists "relay_calendar_events_delete_own" on public.relay_calendar_events;
