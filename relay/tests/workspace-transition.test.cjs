...
test('a loading page stays hidden until its data is ready',()=>{const s=setup();s.controller.holdForLoading(true);assert.equal(s.phase,'hidden');s.controller.request('/chats/');s.advance(PANEL_CLOSE_MS);s.commit('/chats/');s.advance(PANEL_OPEN_MS);assert.equal(s.phase,'hidden');s.controller.holdForLoading(false);s.advance(PANEL_OPEN_MS);assert.equal(s.phase,'open')});
