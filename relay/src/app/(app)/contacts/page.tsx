...
export default function ContactsPage() {
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-8 md:px-6">
        ...
        <div className="mt-6 rounded-md border border-border p-5">
          ...
          <button type="button" onClick={() => window.location.reload()} className="mt-4 rounded-md bg-ink px-4 py-2 text-sm font-medium text-canvas">Reload</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 md:px-6">
      ...
      <div className="mt-6 grid grid-cols-3 gap-1 rounded-lg border border-border bg-surface p-1" role="tablist" aria-label="Contacts sections">
      ...
      {tab === 'contacts' && (
        <section className="mt-6" role="tabpanel">
          {state.incoming.length > 0 && (
            <button type="button" onClick={() => setTab('requests')} className="mb-4 flex w-full items-center justify-between rounded-md border border-border bg-surface px-3 py-2.5 text-left transition-colors hover:bg-surface-raised">
        ...
        <section className="mt-6" role="tabpanel">
          ...
          {requestCount === 0 ? (
            <div className="rounded-md border border-dashed border-border px-4 py-10 text-center">
              ...
              <button type="button" onClick={() => setTab('discover')} className="mt-2 text-xs text-ink-faint underline underline-offset-4 hover:text-ink">Discover people</button>
            </div>
...
function TabButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`relative flex min-w-0 items-center justify-center gap-1.5 rounded-md px-2 py-2 text-xs font-medium transition-colors sm:text-sm ${active ? 'bg-canvas text-ink shadow-sm' : 'text-ink-muted hover:text-ink'}`}
    >
