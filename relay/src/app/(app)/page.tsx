...
export default function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-7 md:px-6 md:py-9">
      ...
      <section aria-label="Daily overview" className="relay-motion-dashboard-strip mt-7 grid grid-cols-3 overflow-hidden rounded-lg border border-border bg-surface-raised">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <DashboardCard index={0} icon={<ListTodo size={18} />} title="Today’s tasks" href="/todo" linkLabel="Open week">
          {state.taskError ? <EmptyState>Tasks could not load.</EmptyState> : state.todos.length === 0 ? <EmptyState>Nothing on your list yet.</EmptyState> : (
            <ul className="space-y-1">
              {state.todos.slice(0, 6).map((todo) => (
                <li key={todo.id} className={`flex items-center gap-3 rounded-md px-1 py-2 transition-colors ${freshTaskId === todo.id ? 'relay-motion-live-row' : ''}`}>
                  <button type="button" disabled={taskBusy === todo.id} onClick={() => toggleTodayTask(todo)} aria-label={todo.completed ? `Mark ${todo.title} incomplete` : `Complete ${todo.title}`} className={`flex h-5 w-5 shrink-0 items-center justify-center rounded border transition-all duration-200 ${todo.completed ? 'border-ink bg-ink text-canvas' : 'border-ink-faint text-transparent'} disabled:opacity-50`}><Check size={13} strokeWidth={3} className={completedPulseId === todo.id ? 'relay-motion-check' : ''} /></button>
                  <span className={`text-sm transition-[color,opacity,text-decoration-color] duration-200 ${todo.completed ? 'text-ink-faint line-through' : 'text-ink'}`}>{todo.title}</span>
                </li>
              ))}
            </ul>
          )}
          ...
          {taskMessage && <p className="relay-motion-row-in mt-2 text-sm text-red-600">{taskMessage}</p>}
        </DashboardCard>

        <DashboardCard index={1} icon={<CalendarDays size={18} />} title="Up next" href="/calendar" linkLabel="Calendar">
        ...
        <DashboardCard index={2} icon={<Mail size={18} />} title="Recent email" href="/email" linkLabel="Inbox">
        ...
        <DashboardCard index={3} icon={<MessageCircle size={18} />} title="Newest chats" href="/chats" linkLabel="Chats">
          {state.chatNotifications.length === 0 ? <EmptyState>No new chat notifications.</EmptyState> : (
            <ul className="divide-y divide-border">
              {state.chatNotifications.map((notification) => (
                <li key={notification.id} className={freshNotificationId === notification.id ? 'relay-motion-live-row' : ''}>
