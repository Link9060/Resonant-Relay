...
const themeInitScript = `(function(){try{var stored=localStorage.getItem('relay-theme');var theme=stored||(window.matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light');document.documentElement.classList.toggle('dark',theme==='dark');var modeMap={relay:'flow',minimal:'still',scifi:'nexus'};var paletteMap={lavender:'violet',ocean:'cyan',sage:'emerald',sunset:'amber',midnight:'cobalt'};var rawMode=localStorage.getItem('relay-experience-mode')||'flow';var rawPalette=localStorage.getItem('relay-experience-palette')||'monochrome';var ex=modeMap[rawMode]||rawMode;var pal=paletteMap[rawPalette]||rawPalette;var intensity=Number(localStorage.getItem('relay-experience-intensity')||55);document.documentElement.dataset.relayExperience=ex;document.documentElement.dataset.relayPalette=pal;document.documentElement.dataset.relayLockIn=String(localStorage.getItem('relay-experience-lock-in')==='true');document.documentElement.style.setProperty('--relay-experience-intensity',String(Math.max(0,Math.min(100,intensity))/100));}catch(e){}})();`;
