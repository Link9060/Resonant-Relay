...
export function ExperienceMenu() {
  ...
  return <div ref={ref} className="relative"><button type="button" onClick={() => setOpen(v => !v)} aria-label="Relay Experience" title="Relay Experience" className="flex h-10 w-10 items-center justify-center rounded-lg text-ink-muted transition-colors hover:bg-surface hover:text-ink md:h-9 md:w-9 md:rounded-md"><Palette size={18}/></button>{open && <div className="relay-popover relay-experience-popover fixed left-3 right-3 top-16 z-50 mx-auto max-w-xl rounded-2xl border border-border bg-canvas p-2 shadow-2xl sm:absolute sm:left-auto sm:right-0 sm:top-11 sm:w-[34rem]"><ExperienceControls/></div>}</div>;
}
