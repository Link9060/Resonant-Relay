...
import { Activity, Ban, Database, FileText, Loader2, MessageCircle, NotebookPen, RefreshCw, ShieldCheck, Users, UsersRound } from 'lucide-react';
...
export function OwnerUserInspector({ user, currentUserId, busy, onSelectUser, onRoleChange, onToggleBan, onForceSignOut, onDeleteUser }: Props) {
  ...
  async function addNote() {
    ...
    setNoteBusy(false);
    if (rpcError) { setError(rpcError.message ?? 'Note could not be added.'); return; }
    setNoteDraft('');
    await load(true);
  }
  ...
  async function deleteNote(noteId: string) {
    ...
    setNoteBusy(false);
    if (rpcError) { setError(rpcError.message ?? 'Note could not be deleted.'); return; }
    await load(true);
  }
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 border-b border-border pb-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-2"><span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-ink-faint">◆ Owner Inspector</span></div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button type="button" disabled={refreshing} onClick={() => void load(true)} className="flex h-9 w-9 items-center justify-center rounded-lg border border-border text-ink-muted hover:bg-canvas disabled:opacity-50" aria-label="Refresh account inspector"><RefreshCw size={15} className={refreshing ? 'animate-spin' : ''} /></button>
        </div>
      </div>
      ...
      <div className="-mx-1 mt-4 overflow-x-auto px-1 pb-1" role="tablist" aria-label="Owner account inspector sections">
        <div className="flex min-w-max gap-1">
          {TABS.map((item) => (
            <button key={item.id} type="button" role="tab" aria-selected={tab === item.id} onClick={() => setTab(item.id)} className={`min-h-10 rounded-lg px-3 text-xs font-medium transition-colors ${tab === item.id ? 'bg-ink text-canvas' : 'text-ink-muted hover:bg-canvas hover:text-ink'}`}>{item.label}</button>
          ))}
        </div>
      </div>

      {error && <div className="mt-4 rounded-xl border border-border bg-canvas p-3 text-sm text-ink">{error}</div>}
      {loading ? <div className="flex min-h-80 items-center justify-center gap-2 text-sm text-ink-muted"><Loader2 size={16} className="animate-spin" />Loading Owner details…</div> : data ? (
        <div className="mt-4">
          ...
          {tab === 'security' && (
            <SecurityTab data={data} user={user} currentUserId={currentUserId} busy={busy} onRoleChange={onRoleChange} onToggleBan={onToggleBan} onForceSignOut={onForceSignOut} onDeleteUser={onDeleteUser} />
          )}
          {tab === 'notes' && <NotesTab notes={data.notes ?? []} draft={noteDraft} setDraft={setNoteDraft} busy={noteBusy} onAdd={() => void addNote()} onDelete={(id) => void deleteNote(id)} />}
        </div>
      ) : null}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Metric label="Messages" value={overview.message_count} />
        <Metric label="Contacts" value={overview.contact_count} />
        <Metric label="Groups" value={overview.group_count} />
        <Metric label="Open reports" value={overview.open_report_count} />
      </div>
...
function ContactsTab({ contacts, onSelectUser }: { contacts: any[]; onSelectUser: (id: string) => void }) {
  ...
  return <ul className="divide-y divide-border overflow-hidden rounded-xl border border-border">{contacts.map((contact) => (
    <li key={contact.id}><button type="button" onClick={() => onSelectUser(contact.id)} className="flex min-h-16 w-full items-center justify-between gap-3 px-3 py-3 text-left hover:bg-canvas"><div className="min-w-0"><p className="truncate text-sm font-medium text-ink">{contact.display_name}</p><p className="mt-1 truncate text-xs text-ink-faint">{formatRelay(contact.relay_number)}{contact.school ? ` · ${contact.school}` : ''}</p></div><div className="shrink-0 text-right"><p className="text-xs text-ink-muted">{Number(contact.mutual_count ?? 0)} mutual</p><p className="mt-1 text-[10px] text-ink-faint">Connected {formatDate(contact.connected_at)}</p></div></button></li>
  ))}</ul>;
}
...
function GroupsTab({ groups }: { groups: any[] }) {
  ...
  return <div className="grid gap-3 sm:grid-cols-2">{groups.map((group) => <section key={group.id} className="rounded-xl border border-border bg-canvas p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-ink">{group.name}</p><p className="mt-1 text-xs capitalize text-ink-faint">{group.role}</p></div><span className="text-xs text-ink-muted">{group.member_count} members</span></div><div className="mt-4 grid grid-cols-2 gap-2 text-xs text-ink-muted"><span>{Number(group.message_count ?? 0).toLocaleString()} messages</span><span className="text-right">{group.last_message_at ? timeAgo(group.last_message_at) : 'No messages'}</span></div></section>)}</div>;
}

function ConversationsTab({ conversations }: { conversations: any[] }) {
  return (
    <div className="space-y-3">
      <div className="rounded-xl border border-border bg-canvas p-4"><div className="flex gap-2"><ShieldCheck size={16} className="mt-0.5 shrink-0 text-ink-muted" /><div><p className="text-sm font-medium text-ink">Conversation metadata only</p><p className="mt-1 text-xs leading-5 text-ink-muted">Owner can see participants, activity, and message counts here. Private message bodies are not exposed through account inspection; reported-message context remains available in Moderation.</p></div></div></div>
      {!conversations.length ? <EmptyState icon={<MessageCircle size={18} />} title="No conversations" body="No Relay conversation metadata is associated with this account." /> : <ul className="divide-y divide-border overflow-hidden rounded-xl border border-border">{conversations.map((conversation) => <li key={conversation.id} className="px-4 py-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-ink">{conversation.group_name || conversation.participants?.filter((p: any) => p.id !== conversation.participants?.[0]?.id).map((p: any) => p.display_name).join(', ') || (conversation.type === 'group' ? 'Group conversation' : 'Direct conversation')}</p><p className="mt-1 truncate text-xs text-ink-faint">{(conversation.participants ?? []).map((p: any) => p.display_name).join(' · ')}</p></div><div className="shrink-0 text-right"><p className="text-xs text-ink-muted">{Number(conversation.message_count ?? 0).toLocaleString()} messages</p><p className="mt-1 text-[10px] text-ink-faint">{conversation.last_message_at ? timeAgo(conversation.last_message_at) : 'No activity'}</p></div></div></li>)}</ul>}
    </div>
  );
}
...
function DataTab({ data }: { data: any }) {
  ...
  return <div className="space-y-4"><section className="rounded-2xl border border-border bg-canvas p-5"><div className="flex items-center gap-2 text-sm font-semibold text-ink"><Database size={16} />Data & storage</div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4"><Metric label="Estimated DB" value={formatBytes(storage.database_bytes_estimated)} /><Metric label="Files" value={formatBytes(storage.file_bytes)} /><Metric label="Total footprint" value={formatBytes(storage.total_bytes)} /><Metric label="File objects" value={storage.file_count ?? 0} /></div>{average > 0 && <p className="mt-4 text-xs text-ink-muted">Relay average: {formatBytes(average)} per user. This account is {ratio < 0.1 ? '<0.1' : ratio.toFixed(1)}× average.</p>}<p className="mt-2 text-[11px] leading-5 text-ink-faint">Database usage is an attribution estimate from user-related row payloads. File bytes come from Supabase Storage object metadata. Postgres index/shared-table overhead cannot be perfectly assigned to one user.</p></section><section className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Metric label="Messages" value={overview.message_count ?? 0} /><Metric label="To-dos" value={overview.todo_count ?? 0} /><Metric label="Conversations" value={overview.conversation_count ?? 0} /><Metric label="Privacy requests" value={overview.privacy_requests ?? 0} /></section></div>;
}

function SecurityTab({ data, user, currentUserId, busy, onRoleChange, onToggleBan, onForceSignOut, onDeleteUser }: any) {
  const sessions = data.sessions ?? [];
  const protectedAccount = user.role === 'owner' || user.id === currentUserId;
  return <div className="space-y-4"><section className="rounded-xl border border-border bg-canvas p-4"><div className="text-sm font-semibold text-ink">Recent sessions</div><p className="mt-1 text-xs text-ink-muted">Device/browser labels are derived from Supabase session user-agent data. Precise location is not shown.</p>{!sessions.length ? <p className="mt-4 text-sm text-ink-muted">No active/recent sessions are currently recorded.</p> : <ul className="mt-3 divide-y divide-border">{sessions.map((session: any) => <li key={session.id} className="py-3"><p className="truncate text-sm font-medium text-ink">{deviceLabel(session.user_agent)}</p><p className="mt-1 text-xs text-ink-faint">Refreshed {session.refreshed_at ? timeAgo(session.refreshed_at) : 'unknown'} · AAL {session.aal ?? 'unknown'}</p></li>)}</ul>}</section><section className="rounded-xl border border-border bg-canvas p-4"><div className="flex items-center gap-2 text-sm font-semibold text-ink"><ShieldCheck size={15} />Owner controls</div><p className="mt-1 text-xs text-ink-muted">Every sensitive Owner action is written to Relay's audit log.</p><label className="mt-4 block text-xs text-ink-muted">Role<select value={user.role} disabled={busy || user.id === currentUserId} onChange={(e) => onRoleChange(e.target.value as AppRole)} className="mt-1.5 min-h-11 w-full rounded-lg border border-border bg-surface px-3 text-sm text-ink disabled:opacity-50">{ROLE_ORDER.map((option) => <option key={option} value={option}>{capitalize(option)}</option>)}</select></label><div className="mt-4 grid gap-2 sm:grid-cols-3"><OwnerAction disabled={busy || protectedAccount} onClick={onToggleBan} icon={<Ban size={14} />}>{user.banned_at ? 'Unban account' : 'Ban account'}</OwnerAction><OwnerAction disabled={busy || protectedAccount} onClick={onForceSignOut}>Force sign out</OwnerAction><OwnerAction disabled={busy || protectedAccount} onClick={onDeleteUser} destructive>Delete account</OwnerAction></div>{protectedAccount && <p className="mt-3 text-xs text-ink-faint">Owner/self accounts are protected from destructive controls here.</p>}</section></div>;
}

function NotesTab({ notes, draft, setDraft, busy, onAdd, onDelete }: any) {
  return <div className="space-y-4"><section className="rounded-xl border border-border bg-canvas p-4"><div className="flex items-center gap-2 text-sm font-semibold text-ink"><NotebookPen size={16} />Owner notes</div><p className="mt-1 text-xs text-ink-muted">Private Owner-only notes. Additions and deletions are audit logged.</p><textarea value={draft} onChange={(e) => setDraft(e.target.value)} maxLength={1000} rows={3} placeholder="Add context for future account reviews…" className="mt-4 w-full resize-none rounded-xl border border-border bg-surface px-3 py-3 text-sm text-ink outline-none placeholder:text-ink-faint focus:border-ink-muted" /><div className="mt-2 flex justify-end"><button type="button" disabled={busy || !draft.trim()} onClick={onAdd} className="min-h-10 rounded-lg bg-ink px-4 text-sm font-medium text-canvas disabled:opacity-40">{busy ? 'Saving…' : 'Add note'}</button></div></section>{!notes.length ? <p className="text-sm text-ink-muted">No Owner notes for this account.</p> : <ul className="space-y-2">{notes.map((note: any) => <li key={note.id} className="rounded-xl border border-border bg-canvas p-4"><p className="whitespace-pre-wrap text-sm leading-6 text-ink">{note.note}</p><div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-ink-faint">{note.created_by_name ?? 'Owner'} · {formatDateTime(note.created_at)}</p><button type="button" disabled={busy} onClick={() => onDelete(note.id)} className="text-xs text-ink-faint hover:text-ink">Delete</button></div></li>)}</ul>}</div>;
}
