      ...
      return <li key={message.id} className={`group flex ${isMine ? 'justify-end' : 'justify-start'} ${spacing} ${freshMessageId === message.id ? 'relay-motion-row-in' : ''} ${removingMessageId === message.id ? 'relay-motion-row-out' : ''}`}>
        <div className={`flex max-w-[92%] items-end gap-2 sm:max-w-[78%] ${isMine ? 'flex-row-reverse' : ''}`}>
          ...
          <div className="min-w-0">
            {!isMine && startsGroup && <p className="mb-1 ml-1 text-[11px] font-medium" style={customized ? { color } : undefined}>{senderName}</p>}
