...
type ProfileMap = Record<string, { id: string; display_name: string; avatar_url: string | null; role: AppRole }>;
...
export function MessageThread({ conversationId, title: initialTitle, isGroup, groupId, currentUserId, participantsById, preferencesById, rolesById, initialMessages, initialReactions, initialPinnedIds, initialMuted }: { conversationId: string; title: string; isGroup: boolean; groupId: string | null; currentUserId: string; participantsById: ProfileMap; preferencesById: PreferenceMap; rolesById: Record<string, 'admin' | 'member'>; initialMessages: Message[]; initialReactions: Reaction[]; initialPinnedIds: string[]; initialMuted: boolean }) {
  ...
  useEffect(() => {
    ...
    const messageChannel = supabase.channel(`messages:${conversationId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `conversation_id=eq.${conversationId}` }, (payload) => {
        ...
        setFreshMessageId(incoming.id);
        window.setTimeout(() => setFreshMessageId((current) => current === incoming.id ? null : current), 300);
      })
  ...
  return <div className="flex h-[calc(100vh-57px)] flex-col md:mx-auto md:w-full md:max-w-6xl md:border-x md:border-border">
    <header className="relative flex items-center justify-between border-b border-border px-4 py-3">
      <div className="flex min-w-0 items-center gap-2"><a href={appPageUrl('/chats')} className="text-ink-muted hover:text-ink md:hidden"><ArrowLeft size={18} /></a><div className="min-w-0"><div className="flex items-center gap-1.5"><h1 className="truncate font-display text-lg font-medium tracking-tight text-ink">{title}</h1><UserRoleBadge role={directOther?.role} /></div>{isGroup && <p className="text-[11px] text-ink-faint">{members.length} members · {Object.values(rolesById).filter((role) => role === 'admin').length} admin{Object.values(rolesById).filter((role) => role === 'admin').length === 1 ? '' : 's'}</p>}</div></div>
    ...
    <div className="flex-1 overflow-y-auto px-4 py-4">{messages.length === 0 ? <p className="mt-10 text-center text-sm text-ink-faint">Say hi.</p> : <ul>{messages.map((message, index) => {
      ...
      const spacing = index === 0 ? '' : startsGroup ? 'mt-4' : 'mt-1';
      ...
      const customized = isGroup && !isMine;
      const canEdit = isMine && Boolean(message.body) && clock - new Date(message.created_at).getTime() <= 15 * 60_000;
      const canUnsend = isMine && clock - new Date(message.created_at).getTime() <= 2 * 60_000;
      const reply = message.reply_to_id ? messageById[message.reply_to_id] : null;
      const messageReactions = REACTIONS.map((emoji) => ({ emoji, rows: reactions.filter((item) => item.message_id === message.id && item.emoji === emoji) })).filter((group) => group.rows.length);

      if (editingId === message.id) return <li key={message.id} className={`flex justify-end ${spacing}`}><form onSubmit={(event) => saveEdit(event, message)} className="w-full max-w-[85%] rounded-lg border border-border bg-surface-raised p-3"><textarea autoFocus value={editValue} onChange={(event) => setEditValue(event.target.value)} maxLength={4000} rows={2} className="w-full resize-none bg-transparent text-sm text-ink outline-none" />{editError && <p className="mt-2 text-xs text-red-500">{editError}</p>}<div className="mt-2 flex justify-end gap-2"><button type="button" onClick={() => setEditingId(null)} className="rounded-md px-3 py-1.5 text-xs text-ink-muted">Cancel</button><button type="submit" disabled={editSaving || !editValue.trim()} className="inline-flex items-center gap-1.5 rounded-md bg-ink px-3 py-1.5 text-xs font-medium text-canvas disabled:opacity-40"><Check size={13} />{editSaving ? 'Saving…' : 'Save'}</button></div></form></li>;

      return <li key={message.id} className={`group flex ${isMine ? 'justify-end' : 'justify-start'} ${spacing} ${freshMessageId === message.id ? 'relay-motion-row-in' : ''} ${removingMessageId === message.id ? 'relay-motion-row-out' : ''}`}>
        <div className={`flex max-w-[92%] items-end gap-2 sm:max-w-[72%] lg:max-w-[68%] ${isMine ? 'flex-row-reverse' : ''}`}>
          {!isMine && startsGroup ? <div className="relative h-7 w-7 shrink-0 overflow-hidden rounded-full bg-surface text-[10px] font-semibold text-ink-muted">{sender?.avatar_url ? <Image src={sender.avatar_url} alt="" fill sizes="28px" className="object-cover" unoptimized /> : <span className="flex h-full w-full items-center justify-center">{senderName[0]?.toUpperCase()}</span>}</div> : !isMine ? <div className="w-7 shrink-0" /> : null}
          <div className="min-w-0">
            {!isMine && startsGroup && <div className="mb-1 ml-1 flex items-center gap-1.5 text-[11px] font-medium"><span style={customized ? { color } : undefined}>{senderName}</span><UserRoleBadge role={sender?.role} /></div>}
            <div className={`flex items-center gap-1.5 ${isMine ? 'flex-row-reverse' : ''}`}>
              <div className={`min-w-0 overflow-hidden rounded-2xl px-3 py-2 text-sm leading-5 ${isMine ? 'rounded-br-md bg-ink text-canvas' : customized ? 'rounded-bl-md' : 'rounded-bl-md bg-surface-raised text-ink'}`} style={!isMine && customized ? { backgroundColor: `${color}18`, color, boxShadow: `inset 0 0 0 1px ${color}35` } : undefined}>
                {reply && <button type="button" onClick={() => document.getElementById(`message-${reply.id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })} className={`mb-2 block w-full rounded-md border-l-2 px-2 py-1.5 text-left text-[11px] ${isMine ? 'border-canvas/40 bg-canvas/10 text-canvas/75' : 'border-ink-faint bg-canvas/60 text-ink-muted'}`}><span className="block truncate">Replying to {reply.sender_id === currentUserId ? 'yourself' : participantsById[reply.sender_id]?.display_name ?? 'message'}</span><span className="block truncate opacity-80">{reply.body || 'Attachment'}</span></button>}
                <div id={`message-${message.id}`}>{message.body && <p className="whitespace-pre-wrap break-words">{message.body}</p>}{message.attachments?.length ? <div className={message.body ? 'mt-2' : ''}><AttachmentList attachments={message.attachments} urls={attachmentUrls} isMine={isMine} /></div> : null}</div>
              </div>
              <MessageMenuButton open={openMenuId === message.id} onToggle={() => setOpenMenuId((current) => current === message.id ? null : message.id)} />
            </div>
            <div className={`mt-1 flex flex-wrap items-center gap-1 ${isMine ? 'justify-end pr-10' : 'pl-10'}`}>
              {pinnedIds.has(message.id) && <span className="inline-flex items-center gap-1 rounded-full bg-surface px-2 py-0.5 text-[10px] text-ink-faint"><Pin size={9} />Pinned</span>}
              {message.edited_at && <span className="text-[10px] text-ink-faint">Edited</span>}
              {endsGroup && <span className="text-[10px] text-ink-faint">{new Date(message.created_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</span>}
              {messageReactions.map(({ emoji, rows }) => { const active = rows.some((row) => row.user_id === currentUserId); const pulse = reactionPulse === `${message.id}:${emoji}`; return <button key={emoji} type="button" onClick={() => void changeReaction(message.id, emoji)} className={`inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[11px] transition-[transform,background-color,border-color] ${active ? 'border-ink/20 bg-surface-raised' : 'border-border bg-surface'} ${pulse ? 'relay-motion-pop' : ''}`}>{emoji}{rows.length > 1 && <span className="text-[10px] text-ink-faint">{rows.length}</span>}</button>; })}
            </div>
            {openMenuId === message.id && <div className={`relay-popover mt-1 flex flex-wrap gap-1 rounded-lg border border-border bg-surface-raised p-1.5 shadow-lg ${isMine ? 'ml-auto mr-10' : 'ml-10'} w-fit max-w-[calc(100vw-5rem)]`}>
              <button type="button" onClick={() => { setReplyTo(message); setOpenMenuId(null); }} className="message-action"><CornerUpLeft size={13} />Reply</button>
              {REACTIONS.map((emoji) => <button key={emoji} type="button" onClick={() => void changeReaction(message.id, emoji)} className="flex h-8 w-8 items-center justify-center rounded-md hover:bg-surface">{emoji}</button>)}
              <button type="button" onClick={() => void togglePin(message.id)} className="message-action">{pinnedIds.has(message.id) ? <PinOff size={13} /> : <Pin size={13} />}{pinnedIds.has(message.id) ? 'Unpin' : 'Pin'}</button>
              {canEdit && <button type="button" onClick={() => { setEditingId(message.id); setEditValue(message.body); setEditError(null); setOpenMenuId(null); }} className="message-action"><Pencil size={13} />Edit</button>}
              <button type="button" onClick={() => setReportTarget({ messageId: message.id, userId: message.sender_id, label: senderName })} className="message-action"><Flag size={13} />Report</button>
              {isMine && canUnsend ? <button type="button" onClick={() => void unsend(message.id)} className="message-action text-red-500"><Trash2 size={13} />Unsend</button> : <button type="button" onClick={() => void deleteForMe(message.id)} className="message-action text-red-500"><Trash2 size={13} />Hide</button>}
            </div>}
          </div>
        </div>
      </li>;
...
function GroupControls({ title, members, availableContacts, currentUserId, currentIsAdmin, rolesById, preferencesById, error, onClose, onRename, onAdd, onPromote, onRemove, onReport }: { title: string; members: ProfileMap[string][]; availableContacts: ProfileMap[string][]; currentUserId: string; currentIsAdmin: boolean; rolesById: Record<string, 'admin' | 'member'>; preferencesById: PreferenceMap; error: string | null; onClose: () => void; onRename: () => void; onAdd: (id: string) => void; onPromote: (id: string) => void; onRemove: (id: string) => void; onReport: (member: ProfileMap[string]) => void }) { return <><button type="button" aria-label="Close group controls" className="fixed inset-0 z-30 cursor-default" onClick={onClose} /><div role="dialog" aria-label={`${title} group controls`} className="relay-popover absolute right-4 top-[calc(100%+0.5rem)] z-40 w-[min(25rem,calc(100vw-2rem))] overflow-hidden rounded-lg border border-border bg-surface-raised shadow-xl"><div className="flex items-center justify-between border-b border-border px-4 py-3"><div><p className="text-sm font-medium text-ink">Group controls</p><p className="text-xs text-ink-faint">Group roles and Relay staff titles are shown separately.</p></div><button type="button" onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-md text-ink-muted"><X size={16} /></button></div>{currentIsAdmin && <div className="border-b border-border px-4 py-3"><div className="flex items-center justify-between"><button type="button" onClick={onRename} className="text-xs font-medium text-ink-muted hover:text-ink">Rename group</button>{availableContacts.length > 0 && <select defaultValue="" onChange={(event) => { if (event.target.value) onAdd(event.target.value); }} aria-label="Add a contact to group" className="rounded-md border border-border bg-canvas px-2 py-1.5 text-xs text-ink"><option value="" disabled>Add contact…</option>{availableContacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.display_name}</option>)}</select>}</div></div>}{error && <p className="px-4 pt-3 text-xs text-red-500">{error}</p>}<ul className="max-h-80 overflow-y-auto p-2">{members.map((member) => { const preference = preferencesById[member.id]; const name = member.id === currentUserId ? member.display_name : contactDisplayName(member, preference); const isAdmin = rolesById[member.id] === 'admin'; return <li key={member.id} className="flex items-center gap-3 rounded-md px-2 py-2"><div className="relative flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-surface text-xs font-semibold">{member.avatar_url ? <Image src={member.avatar_url} alt="" fill sizes="36px" className="object-cover" unoptimized /> : name[0]?.toUpperCase()}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><p className="truncate text-sm font-medium text-ink">{name}{member.id === currentUserId && <span className="ml-1 font-normal text-ink-faint">(you)</span>}</p><UserRoleBadge role={member.role} /></div><p className="flex items-center gap-1 text-[11px] text-ink-faint">{isAdmin ? <><Shield size={11} />Group admin</> : 'Group member'}</p></div>{member.id !== currentUserId && <div className="flex gap-1">{currentIsAdmin && !isAdmin && <button type="button" onClick={() => onPromote(member.id)} className="rounded-md px-2 py-1 text-[11px] text-ink-muted hover:bg-surface">Make admin</button>}{currentIsAdmin && <button type="button" onClick={() => onRemove(member.id)} className="rounded-md px-2 py-1 text-[11px] text-red-500 hover:bg-surface">Remove</button>}<button type="button" onClick={() => onReport(member)} aria-label={`Report ${name}`} className="flex h-7 w-7 items-center justify-center rounded-md text-ink-faint hover:bg-surface"><Flag size={13} /></button></div>}</li>; })}</ul></div></>; }
