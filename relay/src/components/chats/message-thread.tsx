...
export function MessageThread({
  ...
  useEffect(() => {
    ...
    // Realtime subscription scoped to this conversation only — Postgres RLS
    // still applies to realtime, so this can't receive rows the viewer
    // isn't a participant in even if the filter were tampered with.
    ...
    <div className="flex h-[calc(100vh-57px)] flex-col md:h-screen">
      <header className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          <Link href="/chats" className="text-ink-muted hover:text-ink md:hidden">
            <ArrowLeft size={18} />
          </Link>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {messages.length === 0 ? (
          <p className="mt-10 text-center text-sm text-ink-faint">Say hi.</p>
        ) : (
              ...
              return (
                <li key={message.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[75%] ${isMine ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
                    {isGroup && !isMine && (
                      <span className="px-1 text-[11px] text-ink-faint">{sender?.display_name}</span>
                    )}
                    <div
                      className={`rounded-lg px-3 py-2 text-sm ${
                        isMine ? 'bg-ink text-canvas' : 'bg-surface-raised text-ink'
                      }`}
                    >
                      {message.body}
                    </div>
