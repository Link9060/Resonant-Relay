...
export interface CreatePlanInput {
  groupId: string; name: string; notes: string; responseType: 'rsvp' | 'select_option' | 'custom_text'; options: string[]; responsePrompt: string;
...
export async function createPlan(input: CreatePlanInput): Promise<ActionResult<{ planId: string }>> {
  const { data, error } = await createClient().rpc('create_plan_v2', {
...
export async function submitPlanResponse(instanceId: string, response: { optionId: string } | { rsvpStatus: 'yes' | 'no' | 'maybe' } | { textResponse: string }): Promise<ActionResult> {
